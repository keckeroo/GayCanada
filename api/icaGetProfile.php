...
$userid  = $_REQUEST['userid'] ? $_REQUEST['userid'] : $_REQUEST['userid2'];
$passkey = $_REQUEST['passkey'] ? $_REQUEST['passkey'] : "";
...
//
// Some backwards compatibility error handling
//
$count = _mysql_get_records("SELECT UserID, Account, DateEntered FROM ACCOUNTS WHERE UserID = '$userid'", &$accountinfo);

if ($count > 0) {
   // Account found - let's see if we have a profile record ..
   $count = _mysql_get_records("SELECT * FROM PROFILES WHERE UserID = '$userid'", &$profileinfo);
   if ($count == 0) {
      // ok - we need to insert a profile stub for this account ......
      $userid = $accountinfo[0]['UserID'];
      $account = $accountinfo[0]['Account'];
      $date  = $accountinfo[0]['DateEntered'];
      _mysql_do("INSERT INTO PROFILES (`UserID`, `Account`, `DateCreated`, `DateModified`) VALUES ('%s', '%s', '%s', '%s')", $userid, $account, $date, $date);
   }
}

$count = _mysql_get_records("SELECT ACCOUNTS.UserID as MainID, ACCOUNTS.Sign, ACCOUNTS.Community, ACCOUNTS.Gender AS Sex, ACCOUNTS.Refresh as Last_Seen, 
                                    floor( DATEDIFF(NOW(),ACCOUNTS.Birthdate )/ 365.25 ) AS AccountAge,
                                    ACCOUNTS.Birthdate as ComputeSign,
                                    ACCOUNTS.Country, ACCOUNTS.City, ACCOUNTS.Province, ACCOUNTS.Service, 
                                    STATUS_UPDATES.StatusUpdate,
                                    PROFILES.*, DATE_FORMAT(PROFILES.DateCreated, '%b %d, %Y') AS Date_Created,
                                    DATE_FORMAT(PROFILES.DateModified, '%b %d, %Y') AS Date_Modified,
                                    geobytes.Cities.city as geocity, geobytes.Cities.region as georegion, geobytes.Cities.country as geocountry
                             ...
                             WHERE 
                               ...
                               AND (PROFILES.SystemStatus != 'Inactive' OR PROFILES.SystemStatus IS NULL)
...
//   $result = _mysql_do("UPDATE PICTURES SET `enabled` = 'Yes' WHERE Account = '$accountid' AND ID in ($pics)");   
   // update friends list in new table ...

   if ($dataset[0]['ComputeSign']) {
      list($year,$month,$day)=explode("-",$dataset[0]['ComputeSign']);
   ...
   if ($dataset[0]['LinkFriends'] != '') {
      // member has friends ... let's add them to database
      $friends = explode(',', $dataset[0]['LinkFriends']);
      $account = $dataset[0]['Account'];
      foreach ($friends as $value) {
          _mysql_do("REPLACE INTO MEMBERLISTS (`account`, `userid`, `group`) VALUES ('%s', '%s', 'Friends')", $account, $value);
      }
   }
}


if ($dataset[0]["AdultSharing"] == 1 && ($dataset[0]["Breast"] || $dataset[0]["PenisLength"] || $dataset[0]["PenisGirth"] || $dataset[0]["Foreskin"] || $dataset[0]["CutUncut"] || $dataset[0]["Pubic"] || $dataset[0]["Role"] || $dataset[0]["SexualFrequency"] || $dataset[0]["BodyHair"] || $dataset[0]["SexualActivity"])) {
...
$success = $count > 0;
if ($count == 0) {
   $errormsg = 'Unable to find profile for '. $userid . '.';
   $keyreqd  = false;
}
else {
   if ($dataset[0]['Sharing'] == 'Private' && $dataset[0]['PassKey']) {
       ...
       if ($passkey && $passkey == $dataset[0]['PassKey']) {
          ...
          $prid = $dataset[0]['PRID'];
          if ($REMOTE_USER && $REMOTE_USER != $userid) {
...
$returnString['REMOTEUSER'] = $REMOTE_USER;
        ...
        ?>  </TD>
            <TD ALIGN=LEFT class=dashline BGCOLOR=#FFFFFF><?php
                ...
                <tr><td class=bodytext><B>Gender:</B> <?=$sex_code[$profile[0]["Gender"]] ?> &nbsp;&nbsp;<B>Age:</B> <?= $profile[0]["Age"] ?>&nbsp;&nbsp;<?=$smoker_status?>
                    ...
                    <TD align=center rowspan=2><? if ($profile[0]["Sign"]) { ?><div style='background-color:#5D190B; border:1px solid #940000; width:150px; color:white;'><img align=top src="http://images.gaycanada.com/profiles/<?php echo(strtolower($profile[0]["Sign"])); ?>.gif"> <?= $profile[0]["Sign"] ?></div><? } ?>
