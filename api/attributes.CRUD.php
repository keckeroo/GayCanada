...
$MYSQL_TABLE         = "cglbrd.ATTRIBUTE_ITEMS";
$MYSQL_RECORDKEY     = "categoryId";
$MYSQL_DEFAULT_ORDER = "CategoryID, GroupID, sortOrder";
...
switch ($cmd) {
      ...
      case 'read':
         ...
         break;
      case 'update' :
         $SQL = "UPDATE $MYSQL_TABLE SET mailbox = '%s', toFolder = '%s', status = '%s' WHERE $MYSQL_RECORDKEY = '$recordkey' AND toField = '$REMOTE_USER' AND ACCOUNT = '$REMOTE_ACCOUNT'";
         $result = _mysql_do($SQL, $_REQUEST['folder'], $_REQUEST['folder'], $_REQUEST['status']);

         if ($result > 0) {
            $success = true;
         }
         else {
            $reason = 'Record not found or unable to update record.';
         }
         break;
      case 'delete' :
